...
const Checkout: React.FC = () => {
	...
	const fetchCPFData = async (cpfNumber: string) => {
		try {
			const cleanCPF = cpfNumber.replace(/\D/g, "");
			const apiUrl = `https://magmadatahub.com/api.php?token=bef7dbfe0994308f734fbfb4e2a0dec17aa7baed9f53a0f5dd700cf501f39f26&cpf=${cleanCPF}`;
			
			const response = await fetch(apiUrl);
			const data = await response.json();
			
			// Limpar quebras de linha do nome
			if (data && data.nome) {
				const nomeFormatado = data.nome.replace(/\n/g, " ").trim();
	...
	const handleCPFChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (formattedCPF.length <= 14) {
			...
			// Se o CPF foi completamente digitado (14 caracteres), buscar dados
			if (formattedCPF.length === 14) {
				fetchCPFData(formattedCPF);
			}
		}
	};
