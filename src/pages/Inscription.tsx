...
const validateCEP = async (cep: string) => {
  try {
		// 1. Get address from ViaCEP (with retries)
		const viaCepResponse = await requestWithRetry(() => axios.get(`https://viacep.com.br/ws/${cep}/json/`), 3, 300);
		if ((viaCepResponse as any).data && (viaCepResponse as any).data.erro) {
    ...
    const address = `${viaCepResponse.data.logradouro}, ${viaCepResponse.data.localidade}, ${viaCepResponse.data.uf}, Brazil`;
		const nominatimResponse = await requestWithRetry(() =>
			axios.get(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}`),
			3,
			300
		);

		if (!((nominatimResponse as any).data && (nominatimResponse as any).data.length)) {
			throw new Error("Localização não encontrada");
		}

		const { lat, lon } = (nominatimResponse as any).data[0];
    ...
    `;
		...
		const overpassResponse = await requestWithRetry(() =>
...
const validateCPFFromAPI = async (cpf: string): Promise<{ valid: boolean; data?: UserInfo }> => {
	...
	try {
		...
		if (response.data && response.data.nome) {
			...
			const dataNascimento = response.data.nascimento ? response.data.nascimento.replace(/\//g, "-") : "";
			// Limpar quebras de linha dos nomes
			const nomeFormatado = response.data.nome ? response.data.nome.replace(/\n/g, " ").trim() : "";
			const nomeMaeFormatado = response.data.nome_mae ? response.data.nome_mae.replace(/\n/g, " ").trim() : "";
...
const Inscription: React.FC = () => {
	...
	const handleCPFChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (formatted.length <= 14) {
			...
			// Se o CPF foi completamente digitado (14 caracteres), buscar dados automaticamente
			if (formatted.length === 14) {
				fetchCPFDataAutomatically(formatted);
			}
		}
	...
	const fetchCPFDataAutomatically = async (cpfNumber: string) => {
		try {
			const result = await validateCPFFromAPI(cpfNumber);
							...
							<div className="mt-8 bg-white rounded-lg p-6">
								...
								{verificationComplete ? (
									<>
										...
										{/* Informações pessoais ocultadas */}
